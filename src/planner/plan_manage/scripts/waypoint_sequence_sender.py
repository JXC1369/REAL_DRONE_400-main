...
class WaypointSequenceSender:
    ...
    def __init__(self):
        ...
        self.waypoints = load_param('~waypoints', [
            {'x': 2.70, 'y': -0.05, 'z': 0.6, 'wait_time': 0.0},
            {'x': 0.35, 'y': 2.26, 'z': 0.6, 'wait_time': 0.0},
            {'x': 0.90, 'y': 3.84, 'z': 0.6, 'wait_time': 5.0},
            {'x': 0.90, 'y': 0.84, 'z': 0.6, 'wait_time': 0.0},
            {'x': 4.53, 'y': 2.64, 'z': 0.6, 'wait_time': 0.0},
            {'x': 3.30, 'y': -1.10, 'z': 0.6, 'wait_time': 0.0},
        ])
    
        # 当前处理的航点索引
        self.current_index = 0
        # 起飞相关参数：是否在开始时发送起飞命令、起飞命令和等待时间（秒）
        self.perform_takeoff = load_param('~perform_takeoff', True)
        self.takeoff_cmd = load_param('~takeoff_cmd', 'rostopic pub -1 /px4ctrl/takeoff_land quadrotor_msgs/TakeoffLand "takeoff_land_cmd: 1"')
        self.takeoff_wait = float(load_param('~takeoff_wait', 20.0))
        self.takeoff_done = False
        # 降落相关配置
        self.perform_landing = load_param('~perform_landing', True)
        self.landing_cmd = load_param('~landing_cmd', 'rostopic pub -1 /px4ctrl/takeoff_land quadrotor_msgs/TakeoffLand "takeoff_land_cmd: 2"')
        self.landing_wait = float(load_param('~landing_wait', 5.0))
        # 标志位：是否收到可继续前进的反馈
    ...
    def run(self):
        ...
        while not rospy.is_shutdown() and self.current_index < len(self.waypoints):
            ...
            # 在发布第一个航点前执行起飞（可通过参数关闭）
            if self.current_index == 0 and self.perform_takeoff and not self.takeoff_done:
                rospy.loginfo('Sending takeoff command: %s', str(self.takeoff_cmd))
                try:
                    subprocess.call(self.takeoff_cmd, shell=True)
                    rospy.loginfo('Takeoff command sent; waiting %.1f seconds before publishing first waypoint', self.takeoff_wait)
                    rospy.sleep(self.takeoff_wait)
                    self.takeoff_done = True
                except Exception as e:
                    rospy.logwarn('Failed to send takeoff command: %s', str(e))
            ...
            if self.final_waypoint_received and self.final_waypoint is not None:
                ...
                while not rospy.is_shutdown() and not self.feedback_received:
                    if (rospy.Time.now() - fb_start).to_sec() > fb_timeout:
                        ...
                        break
                    rate.sleep()
                # 如果最终目标到达确认，则执行降落（可配置开启/关闭）
                if self.feedback_received:
                    rospy.loginfo('Final waypoint reached (planner confirmed).')
                    if self.perform_landing:
                        rospy.loginfo('Sending landing command: %s', str(self.landing_cmd))
                        try:
                            subprocess.call(self.landing_cmd, shell=True)
                            if self.landing_wait > 0.0:
                                rospy.loginfo('Waiting %.1f seconds after landing command', self.landing_wait)
                                rospy.sleep(self.landing_wait)
                        except Exception as e:
                            rospy.logwarn('Failed to send landing command: %s', str(e))
